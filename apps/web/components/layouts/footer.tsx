...
export function Footer({ locale }: { locale: Locale }) {
  ...
  return (
    <footer className="mt-auto border-t border-border/40 bg-background pb-24 pt-10 lg:pb-10">
      <div className="container-umq">
        ...
        >
          <div className="flex flex-wrap items-center justify-center gap-3">
            {socials.map(({ href, label, icon: Icon }) => (
              <a
                ...
                aria-label={label}
                className="flex h-10 w-10 items-center justify-center rounded-full text-foreground-muted transition-colors hover:bg-muted/30 hover:text-foreground"
              >
          ...
          <p className="text-center text-sm leading-relaxed text-foreground-muted">
            © {year} /{dict.brand}/{" "}
            {locale === "ar" ? "رقم السجل" : "CR"} {CR_NUMBER}
          </p>
        </motion.div>
      </div>
    </footer>
